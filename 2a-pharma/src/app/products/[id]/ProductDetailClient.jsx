...
const labels = {
  ...
  en: { notFound: "Product not found", back: "Back to products", backBtn: "Go back", related: "Related products", contactPrice: "Contact us for price", sendEmail: "Send email" },
  it: { notFound: "Prodotto non trovato", back: "Torna ai prodotti", backBtn: "Indietro", related: "Prodotti correlati", contactPrice: "Contattaci per il prezzo", sendEmail: "Invia email" },
};
...
export default function ProductDetailClient() {
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        if (!docSnap.exists()) { setProduct(null); setLoading(false); return; }
        const p = { id: docSnap.id, ...docSnap.data() };
        setProduct(p);
        const snap = await getDocs(collection(db, "products"));
        const all = snap.docs.map(d => ({ id: d.id, ...d.data() }));
        setRelated(all.filter(r => r.category_en === p.category_en && r.id !== p.id).slice(0, 3));
      } catch (err) {
        console.error(err);
        setProduct(null);
      }
  ...
  if (!product) return (
    <div className={styles.notFound}>
      ...
      <h2>{lb.notFound}</h2>
      <Link href="/products" className={styles.backBtn}><IconArrowLeft /> {lb.back}</Link>
    </div>
  );
      ...
      <div className={styles.mainGrid}>
        ...
        <div className={styles.info}>
          ...
          <div className={styles.cta}>
            <a href="tel:+355684083950" className={styles.ctaBtnPrimary}><IconPhone /> {lb.contactPrice}</a>
            <a href="mailto:info@2apharma.al" className={styles.ctaBtnOutline}><IconMail /> {lb.sendEmail}</a>
          </div>
        </div>
      </div>
